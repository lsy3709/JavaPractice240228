...
public class Ex_01_Socket_singlechat_server {
	...
	// 준비물 1) 서로 역할 , 누가 : 서버, 누가2 : 클라이언트
	
	// 서버역할 : 상대방 클라이언트에게, 본인의 아이피 
	// 기본 윈도우즈 방화벽이 설정. 해제 
	// 검색 : 고급 보안이 포함된 Windows Defender 방화벽 앱이 실행. 키워드 : 방화벽 
	// 순서1 ) 인바운드 규치 우클릭 -> 새규칙 클릭. 
	// 순서2 ) 포트 클릭 
	// 순서3 ) TCP 체크, 특정 포트 : 9999
	// 순서4 ) 연결허용 클릭. 
	// 순서5 ) 도메인, 공용, 개인 다 체크 다음. 
	// 순서6 ) 이름 : 1_1_채팅 연습 with 짝지 포트 9999, 완료 
	
	//예) win + r -> cmd -> ipconfig -> 10.100.104.27, 이 아이피 알려주기. 추가로 포트 : 9999
	// 파일명 서버 , 실행
	
	// 클라이언트 역할 : 서버의 아이피 주소와 포트를 이용해서, 변경. 
	// socket = new Socket("localhost",9999); 수정이 필요
	// socket = new Socket("10.100.104.27",9999);
	// 파일명 클라이언트, 실행. 
